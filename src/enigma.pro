%% Enigma cipher in SWI-Prolog (proper .pro extension)
:- module(enigma, [enigma/3]).
...
list_nth([], _, 0).
list_nth([H|_], 0, H) :- !.
list_nth([_|T], N, E) :- N1 is N - 1, list_nth(T, N1, E).

rotor_pass(Id, Dir, C, Pos, Out) :-
    (Dir = fwd -> rotor_fwd(Id, W) ; rotor_bwd(Id, W)),
    mod26(C + Pos, Idx), list_nth(W, Idx, Val), mod26(Val - Pos, Out).

step_rotors([P0,P1,P2], [P0,NP1,NP2]) :-
    notch(1, N1), notch(2, N2),
    (P2 =:= N2 -> mod26(P2+1, TP2) ; TP2 = P2),
    Mid = (P1 =:= N1),
    ((Mid ; TP2 =:= N2) -> mod26(P1+1, NP1) ; NP1 = P1),
    mod26(TP2+1, NP2).

encrypt_one(C, Pos, Out) :-
    Pos = [P0,P1,P2],
    rotor_pass(2,fwd,C,P2,C1), rotor_pass(1,fwd,C1,P1,C2), rotor_pass(0,fwd,C2,P0,C3),
    reflector(Ref), list_nth(Ref, C3, CR),
    rotor_pass(0,bwd,CR,P0,C4), rotor_pass(1,bwd,C4,P1,C5), rotor_pass(2,bwd,C5,P2,Out).

enigma([], _, []).
enigma([H|T], Pos, [OH|OT]) :-
    step_rotors(Pos, NPos),
    encrypt_one(H, NPos, OH),
    enigma(T, NPos, OT).

:- initialization((
    enigma([7,4,11,11,14,22,14,17,11,3], [0,0,0], Out),
    format("~w~n", [Out])
)).
