// Enigma Cipher - Opa
// Secure web application framework/language
...
function getFwd(r, i) {
    match (r) {
        case 0: List.get(i, fwdI) ? 0
        case 1: List.get(i, fwdII) ? 0
        default: List.get(i, fwdIII) ? 0
    }
}

function getBwd(r, i) {
    match (r) {
        case 0: List.get(i, bwdI) ? 0
        case 1: List.get(i, bwdII) ? 0
        default: List.get(i, bwdIII) ? 0
    }
}
...
Log.info("Enigma Cipher - Opa", "Test: AAAAA -> BDZGO")
