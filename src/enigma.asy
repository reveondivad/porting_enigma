// Enigma cipher in Asymptote (vector graphics language)
...
string enigma(string text) {
  ...
  for (int i = 0; i < length(text); ++i) {
    int c = ascii(substr(text, i, 1)) - 65;
    ...
    pos[2] = mod26(pos[2] + 1);
    for (int j = 2; j >= 0; --j) {
      int[][] fwd = {rotorFwd1, rotorFwd2, rotorFwd3};
      c = rotorPass(fwd[j], c, pos[j]);
    }
    c = reflector[c];
    for (int j = 0; j <= 2; ++j) {
      int[][] bwd = {rotorBwd1, rotorBwd2, rotorBwd3};
      c = rotorPass(bwd[j], c, pos[j]);
    }
    result += chr(c + 65);
  }
  return result;
